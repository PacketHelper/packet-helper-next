...
class TestApi(APITestCase):
    ...
    def test_add_hex(self):
        ...
        url = self.client.get(f"/api/hex/{Hex}")
    ...
    def test_like_hex(self):
        ...
        url = self.client.get(f"/api/hex/{Hex}/like")
        assert Hexes.objects.filter(Hex=Hex.lower()), "Hex should be added to database"
        assert (
            Hexes.objects.get(Hex=Hex.lower()).Likes == likes + 1
        ), "Like should be added to hex"
    ...
    def test_dislike_hex(self):
        ...
        assert Hexes.objects.filter(Hex=Hex.lower()), "Hex should be added to database"
        assert (
            Hexes.objects.get(Hex=Hex.lower()).Dislikes == dislikes + 1
        ), "Dislike should be added to hex"
    ...
    def test_dislike_nonexistent_hex(self):
        ...
        assert not Hexes.objects.filter(
            Hex=Hex.lower()
        ), "Tested hex shouldn't be in the database, this one is"
